...
import kr.debop4j.data.redis.serializer.BinaryRedisSerializer;
import kr.debop4j.data.redis.serializer.RedisSerializer;
import kr.debop4j.data.redis.serializer.SerializationTool;
        ...
        try {
            ...
            runWithTx(new JedisTransactionalCallback() {
                ...
                public void execute(Transaction tx) {
                    ...
                    byte[][] keyArr = keys.toArray(new byte[keys.size()][]);
                    if (keyArr.length > 0) {
                        tx.del(keyArr);
